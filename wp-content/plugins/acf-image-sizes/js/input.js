(function($){
	...
	*/
	...
	acf.fields.image_sizes = {
		...
		add : function( image ){
			...
			// set atts
			div.find('.acf-image-sizes-image').attr( 'src', image.url );
			div.find('.acf-image-sizes-value').val( image.id ).trigger('change');
		...
		remove : function()
		{
			...
			// set atts
		 	this.$el.find('.acf-image-sizes-image').attr( 'src', '' );
			this.$el.find('.acf-image-sizes-value').val( '' ).trigger('change');
					...
					selection.each(function(attachment){
				    	...
				    	if( i > 1 )
						{
							...
							var $td			=	_media.div.closest('td'),
								...
								selector	=	'td .acf-image-sizes-uploader:first';
								
							
							// key only exists for repeater v1.0.1 +
							if( key )
							{
								selector = 'td[data-field_key="' + key + '"] .acf-image-sizes-uploader';
							}
				    	...
				    	// add image to field
				        acf.fields.image_sizes.add( image );
	...
	*/
	...
	$(document).on('click', '.acf-image-sizes-uploader .acf-button-edit', function( e ){
		
		e.preventDefault();
		
		acf.fields.image_sizes.set({ $el : $(this).closest('.acf-image-sizes-uploader') }).edit();
			
	});
	
	$(document).on('click', '.acf-image-sizes-uploader .acf-button-delete', function( e ){
		
		e.preventDefault();
		
		acf.fields.image_sizes.set({ $el : $(this).closest('.acf-image-sizes-uploader') }).remove();
			
	});
	
	
	$(document).on('click', '.acf-image-sizes-uploader .add-image-sizes', function( e ){
		
		e.preventDefault();
		
		acf.fields.image_sizes.set({ $el : $(this).closest('.acf-image-sizes-uploader') }).popup();
