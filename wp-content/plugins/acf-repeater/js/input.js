	...
	*/
	...
	acf.fields.repeater = {
		...
		add : function( $before ){
			...
			if (this.o.layout != "tablenonheader" ) {
				$before.before( '<tr class="acf-rowspacer"><td colspan="3" style="background-color: #f1f1f1 !important;"></td></tr>' );
			}
		...
		remove : function( $tr ){
			...
			$tr.prev('.acf-rowspacer').addClass('acf-remove-item');
			
			setTimeout(function(){
			
				$tr.prev('.acf-rowspacer').remove();	
				$tr.remove();
