...
class acf_controller_post
{
	...
	function admin_head()
	{
		...
		if( $acfs )
		{
			foreach( $acfs as $acf )
			{
				...
				// $position_old = apply_filters('acf/field_group/get_position_old', $acf['options']['position'], $acf['id']);
				// $position_new = apply_filters('acf/field_group/get_position_new', $acf['options']['position'], $acf['id']);
				...
				//update_post_meta($acf['id'], 'position', $position_new);
			}
			// foreach($acfs as $acf)
		}
		// if($acfs)
	...
	*/
	...
	function meta_box_input( $post, $args )
	{
		...
		if ($args['show'])
		{
			if ($args['field_group']['options']['hide'] == 'yes')
			{
				$screen = get_current_screen();
				$copy = 'no';

				if(array_key_exists('copy', $_GET)) {
					$copy = $_GET['copy'];
				}
				
				if ( $screen->base == "post" && $screen->parent_base == "edit" && ($screen->action == "add" || $copy == "yes" )) {
					$class .= ' acf-hiddengroupfield acf-hidden';
					$toggle_class .= ' acf-hiddengroupfield acf-hidden';
					$hide_on_render = true;
				}
			}
		}
...
(function($) {
	...
	<?php if($hide_on_render) {?>
	$('#<?php echo $id; ?>').hide();
	<?php } ?>

	<?php if($args['field_group']['options']['titlerequired'] == 'yes' &&  $args['show'] ) {?>
		$('#post-body-content').addClass('acf_postbox_title no_box');
		$('#titlediv').addClass('field required');
		$('#titlediv').wrap('<div id="titlediv2" class="inside"></div>');
		$('#titlediv2').addClass('inside');
	<?php } ?>
