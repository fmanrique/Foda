...
?>
	...
	*/
	...
	$(document).ajaxComplete(function(event, xhr, settings) {
		...
		if (typeof acfExtraFunctions == 'function') {
			acfExtraFunctions();
		} 
	...
	*-------------------------------------------------------------------------------------*/
	...
	function acf_everything_fields()
	{
		...
		// Show Region and Languages in user edit, only for admin.
		if ( ($options['page_type'] == 'user') && (empty(wp_get_current_user()->caps['administrator']))) {
			if (empty(wp_get_current_user()->caps['editoradmin'])) {
				if (empty(wp_get_current_user()->caps['add_users'])){
					die;
				}
			}
		} 
